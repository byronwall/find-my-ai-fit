...
- After cloning this starter, replace the `example-bare` package name and other visible Starter placeholders with the new project's identity before feature work or deployment.
...
- Treat repo-root `live-server-details.json` as the authoritative server contract. If it says the server is running and advertises a URL, the server **is running**: use that exact URL and do not second-guess it with `curl`, `lsof`, `ps`, port probes, or a competing start command.
- One checkout gets exactly one dev server. Never launch direct `vinxi`, choose an alternate port, remove the server lock/manifest, or start a fallback server while `live-server-details.json` reports a running server. Multiple processes would hot-reload the same files and may mutate the same development data.
- Only when `live-server-details.json` is missing or explicitly reports an inactive/stopped server may you start one. The dev server does not work reliably inside the sandbox; when a start is genuinely needed, request escalation and run `pnpm -C app dev` outside the sandbox.
...
- Before any browser/manual check, read `live-server-details.json` and navigate to its advertised URL. A running status is definitive even if a first browser navigation races startup or briefly fails; retry the advertised URL rather than probing ports or spawning another server.
- Do not run `pnpm -C app dev` merely to confirm the server. Run it only when the manifest is missing or explicitly inactive/stopped; otherwise reuse the advertised process and URL.
...
- Keep `app/pnpm-workspace.yaml` and `app/patches/` in any Docker dependency-install layer so pnpm 11 sees build approvals and patched dependencies before a frozen install.
